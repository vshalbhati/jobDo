...
// Maps whose keys can be deleted must be overwritten wholesale; merging them
// would resurrect every skill removed in Settings.
const REPLACE_WHOLE = new Set(['profile.skills']);

// Deep-merges settings over the defaults, so a release that adds a field
// doesn't break an existing account. Arrays are replaced, not merged. Used by
// the extension for its local copy and by the web app for the account's.
export function mergeConfig(base, over, path = '') {
  ...
  for (const [k, v] of Object.entries(over)) {
    ...
    out[k] = (k in base) ? mergeConfig(base[k], v, p) : v;
  }
  return out;
}
...
export function defaultConfig() {
  return {
    ...
    // The jobDo account. Everything above is edited on the jobDo website and
    // downloaded from the account; this browser only keeps a copy to run on.
    sync: {
      enabled: false,
      serverUrl: 'https://job-do.vercel.app',
      webUrl: 'https://job-do.web.app',   // where the jobDo website is hosted
      email: '',
      token: '',
      refreshToken: '',
      expiresAt: 0,
      autoPush: true,
      lastPushAt: 0,
      lastPullAt: 0,     // when settings and resume were last downloaded
      lastPullError: '',
      migrated: false,   // browser-kept settings have been handed to the account
      lastError: '',
      pending: 0
    },
